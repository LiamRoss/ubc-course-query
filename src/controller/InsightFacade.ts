                        ...
                        if (applyKey[key].constructor === Object) {
                            ...
                            if (["MAX", "MIN", "AVG", "COUNT", "SUM"].indexOf(at) !== -1) {
                                this.validKey(applyToken[at]).then(() => {
                                    // check if type of key matches numerical value for ApplyToken
                                    if (at === "COUNT") {
                                        // TODO: check if the value of this is something that could be a string or number
                                        fulfill();
                                    } else if (at === "MAX" || at === "MIN"  || at === "AVG" || at === "SUM"){
                                        // TODO: check if the value of this is something that should be ONLY a number
                                        fulfill();
                                    } else {
                                        ////Log.trace(at + " is not valid key");
                                        reject(at + " is not valid key");
                                    }
        ...
        return new Promise((fulfill, reject) => {
            try {
                ...
                this.dataTransformer(query, incomingSections).then((groups: Group[]) => {
                    //Log.trace("dataTransformer fulfills");
                    ...
                    //Log.trace("somehow rejects even though fulfilled?");
                    reject(err);
                })
            } catch (error) {
                reject("rejected in formatJsonResponse");
            }
        ...
        return new Promise((fulfill, reject) => {
            try {
                ...
                if (query.hasOwnProperty("TRANSFORMATIONS")) {
                            ...
                            for (let group of groups) {
                                ...
                                if (sectionMatches) {
                                    ...
                                    if (groupIndex !== -1) {
                                        ...
                                        if (Object.keys(returnedMerge).length !== 1 || !returnedMerge.hasOwnProperty('error')) {
                                            groups[groupIndex] = returnedMerge;
                                        } else {
                                            reject(returnedMerge["error"]);
                                        }
                                    } else {
                    ...
                    //Log.trace("groups: \n" + JSON.stringify(groups));
                    //Log.trace("dataTransformer returns w/ groups");
                    fulfill(groups);
                }
                // else, no need to create groups, return validSections
                else {
                    //Log.trace("dataTransformer returns, no groups");
                    fulfill(validSections);
                }
            } catch (error) {
    ...
    mergeSectionGroup(section: Section | Room, group: any, transformations: Transformations): Group {
        ...
        try {
            ...
            if (group.constructor === Object && Object.keys(group).length === 0) {
                ...
                // for each GROUP, set group properties with section of same key
            ...
            for (let applyKey of transformations.APPLY) {
                ...
                switch (applyToken) {
                    ...
                    case "MIN":
                        ...
                        // check if is numbers
                        if (typeof section[sectionKey] !== 'number') {
                            return {"error": "MIN key must be a number"};
                        } else {
                            if (returnGroup.hasOwnProperty(key + "sum")) {
                                returnGroup[key + "sum"] = returnGroup[key + "sum"] + section[sectionKey];
                            } else {
                    ...
                    case "AVG":
                        ...
                        // check if is numbers
                        if (typeof section[sectionKey] !== 'number') {
                            return {"error": "AVG key must be a number"};
                        } else {
                            if (returnGroup.hasOwnProperty(key + "sum")) {
                                returnGroup[key + "sum"] = returnGroup[key + "sum"] + section[sectionKey];
                            } else {
                    ...
                    case "SUM":
                        ...
                        // check if is numbers
                        if (typeof section[sectionKey] !== 'number') {
                            return {"error": "SUM key must be a number"};
                        } else {
                            if (returnGroup.hasOwnProperty(key + "sum")) {
                                returnGroup[key + "sum"] = returnGroup[key + "sum"] + section[sectionKey];
                            } else {
                    ...
                    default:
                        ...
                        return {"error": "defaulted in mergeSectionGroup"};
                }
            }
